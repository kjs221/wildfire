...
const WildfireDashboard = () => {
  ...
  const animals = [
    ...
    { name: 'fennec fox', url: 'https://commons.wikimedia.org/wiki/Category:Vulpes_zerda#/media/File:Fennec_Fox_Peek.jpg?w=500&h=500&fit=crop' },
      ...
      >
        <div className="bg-white p-4 sm:p-8" style={{ borderRadius: '9px' }}>
          ...
          {currentView === 'smoke' && !selectedTask && (
            <div>
              <h2 style={{ fontFamily: "'Ranchers', cursive", fontSize: '24px', color: '#0078a0', marginBottom: '1.5rem' }}>Where There's Smoke (Pipeline)</h2>
              {tasks.length === 0 ? (
                          ...
                          {sortedTasks.map(task => {
                            ...
                            return (
                              <div
                                key={task.id}
                                onClick={() => setSelectedTask(task)}
                                className="p-3 shadow-sm hover:shadow-md cursor-pointer transition-all"
                                style={{
                                  borderLeft: `6px solid ${stream.color}`,
                                  borderRadius: '9px',
                                  background: noDeadline ? '#fefce8' : '#fff',
                                  opacity: isCompleted ? 0.7 : 1,
                                  border: noDeadline ? '1px solid #fde047' : undefined,
                                  borderLeftWidth: '6px',
