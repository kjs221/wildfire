          ...
          {currentView === 'smoke' && !selectedTask && (
            <div>
              ...
              <div className="flex items-center gap-3 mb-4 flex-wrap">
                ...
                >
                  {showBatchArchive ? '✕ Cancel' : 'Batch archive tasks'}
                </button>
          ...
          {currentView === 'hose' && !selectedTask && (
            <div>
              ...
              {tasks.length === 0 ? (
                ...
                // minDate = earliest defined start_date among incomplete tasks, falling back to today
                const definedStartDates = activeTasks
                  .filter(t => t.start_date)
                  .map(t => new Date(t.start_date.split('T')[0]));
                let minDate = definedStartDates.length > 0
                  ? new Date(Math.min(...definedStartDates))
                  : new Date(today);
                minDate.setDate(1); // snap to start of month for clean labels

                // maxDate = latest deadline or subtask deadline, or 6 months out
                const deadlineDates = [];
                activeTasks.forEach(t => {
                  if (t.deadline) deadlineDates.push(new Date(t.deadline));
                  t.subtasks?.forEach(s => { if (s.deadline) deadlineDates.push(new Date(s.deadline)); });
                });
                      ...
                      if (blackoutDays.includes(date.getDay())) {
                        stripes.push(
                          <div
                            key={`bo-${d}`}
                            className="absolute top-0 bottom-0"
                            style={{ left: `${(d / totalDays) * 100}%`, width: `${(1 / totalDays) * 100}%`, background: 'rgba(0,151,178,0.35)', zIndex: 0 }}
                          ></div>
                        );
                      }
                    }
