...
const WildfireDashboard = () => {
  ...
  const [streams, setStreams] = useState([
    { id: '1', name: 'SBAC', color: '#378ADD' },
    { id: '2', name: 'NEF', color: '#BA7517' },
    { id: '3', name: 'Curriculum', color: '#378ADD' },
    { id: '4', name: 'Editing', color: '#639922' },
    { id: '5', name: 'Consulting', color: '#EF8936' },
  ]);
  ...
  const addStream = () => {
    ...
    setStreams([...streams, { id: newId, name: 'New stream', color: '#378ADD' }]);
  };
  ...
  const addHoliday = () => {
    ...
    setHolidays([...holidays, { id: newId, label: 'Time off', start: today, end: today }]);
  };
            ...
            <div className="mb-6">
              ...
              <div className="bg-gray-50 p-6 shadow-md" style={{ borderRadius: '9px' }}>
                <div className="flex justify-between items-start mb-4 gap-4">
                  <h2 style={{ fontFamily: "'Indie Flower', cursive", fontSize: '28px' }} className="text-gray-900">{selectedTask.title}</h2>
                  <button
                    onClick={() => {
                ...
                <div className="mb-6 pb-6 border-b"><h3 style={{ fontFamily: "'Indie Flower', cursive", fontSize: '18px', marginBottom: '0.75rem' }} className="text-gray-900">Subtasks</h3><div className="space-y-2">{selectedTask.subtasks?.map(s => (<label key={s.id} className="flex items-center gap-3 p-3 bg-white" style={{ borderRadius: '9px' }}><input type="checkbox" checked={s.completed} onChange={() => toggleSubtask(selectedTask.id, s.id)} className="w-4 h-4 accent-purple-600" /><span className={`text-sm flex-1 ${s.completed ? 'line-through text-gray-400' : ''}`}>{s.text}</span></label>))}</div></div>
                <div>
                  <h3 style={{ fontFamily: "'Indie Flower', cursive", fontSize: '18px', marginBottom: '0.75rem' }} className="text-gray-900">Notes</h3>
                  <textarea
                    value={taskNotes}
          ...
          {currentView === 'sanity' && !selectedTask && (
            <div>
              ...
              <div className="bg-gray-50 p-6 shadow-md space-y-4" style={{ borderRadius: '9px' }}>
                {streams.map(s => (
                  <div key={s.id} className="flex items-center gap-3 bg-white p-3" style={{ borderRadius: '9px' }}>
                    <input type="color" value={s.color} onChange={(e) => updateStream(s.id, 'color', e.target.value)} className="w-10 h-10 border border-gray-300 cursor-pointer" style={{ borderRadius: '9px' }} />
              ...
              <div className="bg-gray-50 p-6 shadow-md space-y-4" style={{ borderRadius: '9px' }}>
                {holidays.map(h => (
                  <div key={h.id} className="flex items-center gap-3 bg-white p-3 flex-wrap" style={{ borderRadius: '9px' }}>
                    <input type="text" value={h.label} onChange={(e) => updateHoliday(h.id, 'label', e.target.value)} placeholder="Label" className="flex-1 min-w-[120px] px-3 py-2 border border-gray-300 text-sm" style={{ borderRadius: '9px' }} />
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">Start</label>
                      <input type="date" value={h.start} onChange={(e) => updateHoliday(h.id, 'start', e.target.value)} className="px-3 py-2 border border-gray-300 text-sm" style={{ borderRadius: '9px' }} />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">End</label>
                      <input type="date" value={h.end} onChange={(e) => updateHoliday(h.id, 'end', e.target.value)} className="px-3 py-2 border border-gray-300 text-sm" style={{ borderRadius: '9px' }} />
                    </div>
                  ...
                  {streams.map(stream => {
                    ...
                    return (
                      <div key={stream.id} className="flex-shrink-0 w-64 bg-gray-50 p-3" style={{ borderRadius: '9px' }}>
                        <h3 className="text-sm font-semibold mb-3 px-1 flex items-center gap-2" style={{ color: stream.color }}>
          ...
          {currentView === 'hose' && !selectedTask && (
            <div>
              ...
              {tasks.length === 0 ? (
                ...
                let minDate = dates.length ? new Date(Math.min(today, ...dates)) : today;
                let maxDate = dates.length ? new Date(Math.max(today, ...dates)) : new Date(today.getTime() + 30 * 86400000);
                ...
                return (
                  <div className="bg-gray-50 p-4 shadow-md overflow-x-auto" style={{ borderRadius: '9px' }}>
                    <div style={{ minWidth: '600px' }}>
                      <div className="relative h-6 mb-2 border-b border-gray-300">
                        <div className="absolute top-0 bottom-0 w-px bg-red-400" style={{ left: `${(todayOffset / totalDays) * 100}%` }}></div>
                        <span className="absolute text-xs text-red-500 -top-1" style={{ left: `${(todayOffset / totalDays) * 100}%`, transform: 'translateX(-50%)' }}>Today</span>
                        {holidays.map(h => {
                          const hStart = new Date(h.start);
                          const hEnd = new Date(h.end);
                          const startOffset = Math.ceil((hStart - minDate) / 86400000);
                          const endOffset = Math.ceil((hEnd - minDate) / 86400000) + 1;
                          if (endOffset <= 0 || startOffset >= totalDays) return null;
                          const left = Math.max(0, startOffset);
                          const width = Math.min(totalDays, endOffset) - left;
                          return (
                            <div
                              key={h.id}
                              title={h.label}
                              className="absolute top-0 bottom-0"
                              style={{ left: `${(left / totalDays) * 100}%`, width: `${(width / totalDays) * 100}%`, background: 'repeating-linear-gradient(45deg, #f3f4f6, #f3f4f6 4px, #e5e7eb 4px, #e5e7eb 8px)', zIndex: 0 }}
                            ></div>
                          );
                        })}
                      </div>
                      ...
                      <div className="space-y-2">
                        {tasks.map(task => {
                          ...
                          const endOffset = end ? Math.max(startOffset + 1, Math.ceil((end - minDate) / 86400000)) : startOffset + 1;
                          ...
                          return (
                            <div key={task.id} className="flex items-center gap-2">
                              <div className="w-40 flex-shrink-0 truncate text-xs text-gray-700" title={task.title}>{task.title}</div>
                              <div className="relative flex-1 h-6 bg-gray-200" style={{ borderRadius: '9px' }}>
                                {holidays.map(h => {
                                  const hStart = new Date(h.start);
                                  const hEnd = new Date(h.end);
                                  const hStartOffset = Math.ceil((hStart - minDate) / 86400000);
                                  const hEndOffset = Math.ceil((hEnd - minDate) / 86400000) + 1;
                                ...
                                <div
                                  ...
                                  style={{ left: `${leftPct}%`, width: `${Math.max(widthPct, 2)}%`, background: getStreamColor(task.stream_id), borderRadius: '9px', minWidth: '8px', zIndex: 1 }}
                                >
                                  <div className="absolute left-0 top-0 h-full bg-black bg-opacity-20" style={{ width: `${progressPct}%`, borderRadius: '9px 0 0 9px' }}></div>
                                </div>
                              </div>
