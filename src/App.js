...
const WildfireDashboard = () => {
  ...
  const convertToSubtask = async (taskId, parentId) => {
    ...
    try {
      ...
      const subtaskPayload = { project_id: parseInt(parentId), text: selectedTask.title, start_date: selectedTask.start_date || null, deadline: selectedTask.deadline || null, completed: false };
      let res = await fetch(`${SUPABASE_URL}/rest/v1/subtasks`, {
        method: 'POST',
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
      ...
      if (!res.ok) {
        ...
        const { start_date, deadline, ...fallback } = subtaskPayload;
        res = await fetch(`${SUPABASE_URL}/rest/v1/subtasks`, {
          method: 'POST',
          headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        ...
        if (!res.ok) {
          console.error('Failed to create subtask, aborting demotion:', res.status, await res.text());
          return; // Don't delete the project if we couldn't create the subtask
        }
      }
      ...
      // Only delete the project once we've confirmed the subtask was created
                          ...
                          return (
                            <div key={task.id}>
                              ...
                              {subtasksWithDates.map(sub => {
                                const subBar = barStyle(sub.start_date ? sub.start_date.split('T')[0] : getStartDate(task), sub.deadline, '#B5D4F4', sub.completed ? 100 : 0);
                                return (
