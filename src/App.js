          ...
          {currentView === 'hose' && !selectedTask && (
            <div key={localDateStr()}>
              ...
              {tasks.length === 0 ? (
                ...
                console.log('Gantt: activeTasks with start_date:', activeTasks.filter(t => t.start_date).map(t => ({ id: t.id, title: t.title, start_date: t.start_date })));
                console.log('Gantt: definedStartDates count:', definedStartDates.length);
                console.log('Gantt: minDate will be:', definedStartDates.length > 0 ? new Date(Math.min(...definedStartDates)) : 'today fallback');
                let minDate = definedStartDates.length > 0
                  ? new Date(Math.min(...definedStartDates))
                  : new Date(today);
                minDate = new Date(minDate.getFullYear(), minDate.getMonth(), 1);
                console.log('Gantt: minDate after snap:', minDate);
                console.log('Gantt: todayOffset will be:', Math.floor((today - minDate) / 86400000));
                console.log('Gantt: maxDate:', new Date(Math.max(...deadlineDates)), 'sixMonthsOut:', new Date(today.getFullYear(), today.getMonth() + 6, today.getDate()));
                  ...
                  <div className="bg-gray-50 shadow-md" style={{ borderRadius: '9px' }}>
                    ...
                    >
                      <div style={{ width: `${timelineWidth + 240}px`, minWidth: '100%', paddingTop: '4px', paddingBottom: '4px' }}>
                        ...
                        <div className="flex">
                          <div style={{ width: '160px', flexShrink: 0 }}></div>
                          <div className="relative flex-1 border-b border-gray-200" style={{ height: '24px' }}>
                            <div className="absolute top-0 bottom-0 w-px bg-red-400" style={{ left: `${(todayOffset / totalDays) * 100}%` }}></div>
                            <span className="absolute text-xs text-white font-medium px-1.5 py-0.5" style={{ left: `${(todayOffset / totalDays) * 100}%`, transform: 'translateX(-50%)', top: '2px', background: '#E24B4A', borderRadius: '4px', whiteSpace: 'nowrap', zIndex: 3 }}>Today</span>
                          </div>
                          <div style={{ width: '80px', flexShrink: 0 }}></div>
                        </div>
                        {/* Holiday legend */}
