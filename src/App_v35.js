...
const WildfireDashboard = () => {
  ...
  const getDeadlineStyle = (date) => {
    ...
    const days = Math.ceil((new Date(date) - new Date()) / (1000 * 60 * 60 * 24));
    if (days < 0) return { background: 'linear-gradient(90deg, #c4614f 30%, #d4547a 70%)' };
    if (days <= 1) return { background: 'linear-gradient(90deg, #f0a500 30%, #e8935a 70%)' };
    if (days <= 4) return { background: 'linear-gradient(90deg, #7eb8c9 30%, #9b8ec4 70%)' };
    return { background: 'linear-gradient(90deg, #2a9d8f 30%, #106b73 70%)' };
  };
            ...
            <div>
              ...
              <div className="bg-gray-50 p-6 shadow-md space-y-4" style={{ borderRadius: '9px' }}>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={saveSpark} disabled={!sparkForm.title} className="px-6 py-2 text-sm font-semibold text-white disabled:opacity-50" style={{ borderRadius: '9px', background: 'linear-gradient(90deg, #2a9d8f, #34b0ef)' }}>Felt burny. Might delete later.</button>
                </div>
