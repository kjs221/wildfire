          ...
          {currentView === 'hose' && !selectedTask && (
            <div key={localDateStr()}>
              ...
              {tasks.length === 0 ? (
                ...
                // minDate = earliest DEFINED start_date among incomplete tasks only
                // Do NOT use getStartDate() here — that falls back to today and defeats the purpose
                const definedStartDates = activeTasks
                  .filter(t => t.start_date)
                  .map(t => { const d = new Date(t.start_date.split('T')[0] + 'T12:00:00'); return d; });
                let minDate = definedStartDates.length > 0
                  ? new Date(Math.min(...definedStartDates))
                  : new Date(today);
                minDate.setDate(1); // snap to start of month

                const deadlineDates = [];
                activeTasks.forEach(t => {
                  if (t.deadline) deadlineDates.push(new Date(t.deadline.split('T')[0] + 'T12:00:00'));
                  t.subtasks?.forEach(s => { if (s.deadline) deadlineDates.push(new Date(s.deadline.split('T')[0] + 'T12:00:00')); });
                });
