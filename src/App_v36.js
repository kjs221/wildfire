...
const WildfireDashboard = () => {
  ...
  const getDeadlineStyle = (date) => {
    ...
    const days = Math.ceil((new Date(date) - new Date()) / (1000 * 60 * 60 * 24));
    if (days < 0) return { background: 'linear-gradient(135deg, #c4614f 40%, #d4547a 60%)' };
    if (days <= 1) return { background: 'linear-gradient(135deg, #f0a500 40%, #e8935a 60%)' };
    if (days <= 4) return { background: 'linear-gradient(135deg, #7eb8c9 40%, #9b8ec4 60%)' };
    return { background: 'linear-gradient(135deg, #2a9d8f 40%, #34b0ef 60%)' };
  };
  ...
  const navButtons = [
    ...
    { id: 'hose', label: 'Get the Hose', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="0.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10 6h8"/><path d="M12 16h6"/><path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="M8 11h7"/></svg> },
