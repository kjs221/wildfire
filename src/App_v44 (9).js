...
const LifeFiresDashboard = () => {
  ...
  const [logSubview, setLogSubview] = useState('logs'); // 'logs' | 'sparkies'
  ...
  const [dashDateEnd, setDashDateEnd] = useState(() => localDateStr());
  ...
  const animalPool = [
    { slug: 'arctic-fox', count: 9 },
    { slug: 'aussie-possum', count: 2 },
    { slug: 'aye-aye', count: 8 },
    { slug: 'badger', count: 1 },
    { slug: 'bat', count: 6 },
    { slug: 'bear', count: 1 },
    { slug: 'beaver', count: 16 },
    { slug: 'brushtail-possum', count: 3 },
    { slug: 'capybara', count: 1 },
    { slug: 'chinchilla', count: 2 },
    { slug: 'civet', count: 4 },
    { slug: 'cub', count: 17 },
    { slug: 'cuscus', count: 1 },
    { slug: 'duckling', count: 6 },
    { slug: 'emo-cows', count: 2 },
    { slug: 'fennec-fox', count: 4 },
    { slug: 'ferret', count: 2 },
    { slug: 'fluffy-anteater', count: 2 },
    { slug: 'flying-lemur', count: 3 },
    { slug: 'fox', count: 1 },
    { slug: 'honey-badger', count: 7 },
    { slug: 'kinkojou', count: 1 },
    { slug: 'koala', count: 8 },
    { slug: 'koala-wombat-friends', count: 1 },
    { slug: 'lemur', count: 1 },
    { slug: 'lion', count: 3 },
    { slug: 'llama-baby', count: 1 },
    { slug: 'manul-kitten', count: 1 },
    { slug: 'meercat', count: 5 },
    { slug: 'mink', count: 2 },
    { slug: 'mouse-deer', count: 1 },
    { slug: 'muskrat', count: 2 },
    { slug: 'otter', count: 20 },
    { slug: 'owl', count: 4 },
    { slug: 'penguins', count: 9, startAt: 10 },
    { slug: 'pigeon', count: 1 },
    { slug: 'pika', count: 6 },
    { slug: 'pine-marten', count: 7 },
    { slug: 'possum', count: 7 },
    { slug: 'raccoon', count: 5 },
    { slug: 'red-panda', count: 9 },
    { slug: 'rock-hyrax', count: 5 },
    { slug: 'seal-pup', count: 5 },
    { slug: 'serval', count: 4 },
    { slug: 'sloth', count: 8 },
    { slug: 'stoat', count: 7 },
    { slug: 'sugar-glider', count: 4 },
    { slug: 'tanuki', count: 8 },
    { slug: 'tarsier', count: 2 },
    { slug: 'tree-kangaroo', count: 1 },
    { slug: 'wee-hedgie', count: 4 },
    { slug: 'wee-hippo', count: 8 },
    { slug: 'wee-piggy', count: 7 },
    { slug: 'wee-platy', count: 1 },
    { slug: 'wee-porcupine', count: 1 },
    { slug: 'wee-pygmy-hippo', count: 2 },
    { slug: 'wee-skunk', count: 1 },
    { slug: 'wombat', count: 3 },
  ];

  const vowels = ['a','e','i','o','u'];
  const getArticle = (name) => vowels.includes(name[0].toLowerCase()) ? 'an' : 'a';

  const getDisplayName = (slug) => slug.replace(/-/g, ' ');

  const pickRewardAnimal = () => {
    const animal = animalPool[Math.floor(Math.random() * animalPool.length)];
    const start = animal.startAt || 1;
    const num = start + Math.floor(Math.random() * animal.count);
    const padded = animal.startAt ? String(num) : String(num).padStart(2, '0');
    const url = `/animals/${animal.slug}-${padded}.webp`;
    const name = getDisplayName(animal.slug);
    return { name, url, article: getArticle(name) };
  };
  ...
  const fetchStreams = async () => {
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/streams?select=*&order=sort_order.asc`, {
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json' }
      });
      const data = await response.json();
      if (Array.isArray(data) && data.length > 0) {
        const fetched = data.map(s => ({ id: String(s.id), name: s.name, color: s.color, capacity_hours: s.capacity_hours || 0, sort_order: s.sort_order || 0 }));
        setStreams(fetched);
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/settings?key=eq.blackout_days`, {
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json' }
      });
      const data = await response.json();
      if (Array.isArray(data) && data.length > 0) {
        setBlackoutDays(data[0].value);
      }
    } catch (error) {
  ...
  const saveBlackoutDays = async (days) => {
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/settings?key=eq.blackout_days`, {
        method: 'PATCH',
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ value: days })
      });
    } catch (error) {
      console.error('Error saving blackout days:', error);
    }
  };
  useEffect(() => { fetchProjects(); fetchStreams(); fetchHolidays(); fetchSparks(); fetchSettings(); }, []);
  const parseItems = () => {
  ...
  const getDeadlineStyle = (date) => {
    if (!date) return { background: '#888780', color: '#f0f0ef', border: '2px solid #c0c0be' };
    const days = Math.ceil((new Date(date) - new Date()) / (1000 * 60 * 60 * 24));
    if (days < 0) return { background: '#c4614f', color: '#fde8e5', border: '2px solid #f0a090' };
    if (days <= 1) return { background: '#d4891a', color: '#fef4e0', border: '2px solid #f5c97a' };
    if (days <= 4) return { background: '#daf0f5', color: '#1a5c6b', border: '2px solid #7eb8c9' };
    return { background: '#2a9d8f', color: '#e0f5f2', border: '2px solid #80d4cc' };
  };
  ...
  const persistStreamOrder = async (orderedStreams) => {
    for (let i = 0; i < orderedStreams.length; i++) {
      await fetch(`${SUPABASE_URL}/rest/v1/streams?id=eq.${orderedStreams[i].id}`, {
        method: 'PATCH',
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ sort_order: i })
      }).catch(e => console.error('Error saving stream order:', e));
    }
  };
  ...
  const [capacityRangeStart, setCapacityRangeStart] = useState(() => localDateStr());
  ...
  const [manualSubtaskId, setManualSubtaskId] = useState('');
  const [timerSubtaskId, setTimerSubtaskId] = useState('');
  ...
  const toggleSubtaskLog = (subtaskId) => {
    setExpandedSubtaskLogs(prev => ({ ...prev, [subtaskId]: !prev[subtaskId] }));
  };
  ...
  const getStartDate = (task, minDateStr) => {
    if (task.start_date) return task.start_date.split('T')[0];
    return minDateStr || localDateStr();
  };
  ...
  const [batchArchiveSelected, setBatchArchiveSelected] = useState([]);

  const toggleBatchArchiveSelect = (taskId) => {
    setBatchArchiveSelected(prev =>
      prev.includes(taskId) ? prev.filter(id => id !== taskId) : [...prev, taskId]
    );
  };

  const batchArchive = async () => {
    if (batchArchiveSelected.length === 0) return;
    for (const taskId of batchArchiveSelected) {
      await fetch(`${SUPABASE_URL}/rest/v1/projects?id=eq.${taskId}`, {
        method: 'PATCH',
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ archived: true })
      }).catch(e => console.error('Batch archive error:', e));
    }
  ...
  const logTimeEntry = (hours, date, subtaskId = null, startTime = null, endTime = null) => {
    const payload = {
      project_id: selectedTask.id,
      hours,
      entry_date: date,
      notes: '',
      ...(subtaskId ? { subtask_id: parseInt(subtaskId) } : {}),
  ...
  const navButtons = [
    ...
    { id: 'hose', label: 'Get the Hose', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 6h8"/><path d="M12 16h6"/><path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="M8 11h7"/></svg> },
    { id: 'ashes', label: 'Of Fires Past', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.25" strokeLinecap="round" strokeLinejoin="round"><path d="M9 10h.01"/><path d="M15 10h.01"/><path d="M12 2a8 8 0 0 0-8 8v12l3-3 2.5 2.5L12 19l2.5 2.5L17 19l3 3V10a8 8 0 0 0-8-8z"/></svg> },
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: 'linear-gradient(135deg, #0097b2 0%, #0078a0 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      ...
      >
        <div className="bg-white p-4 sm:p-8" style={{ borderRadius: '9px' }}>
          ...
          {currentView === 'log' && !selectedTask && (
            <div>
              <h2 style={{ fontFamily: "'Ranchers', cursive", fontSize: '24px', color: '#0078a0', marginBottom: '1rem' }}>Add a Log to the Fire</h2>
              <div className="flex gap-2 mb-4">
                <button
                  onClick={() => setLogSubview('logs')}
                  className="px-5 py-2 text-sm font-semibold text-white"
                  style={{ borderRadius: '9px', background: logSubview === 'logs' ? 'linear-gradient(90deg, #f9b47f, #cf4384)' : '#e5e7eb', color: logSubview === 'logs' ? '#fff' : '#374151' }}
                >
                  Logs
                </button>
                <button
                  onClick={() => setLogSubview('sparkies')}
                  className="px-5 py-2 text-sm font-semibold"
                  style={{ borderRadius: '9px', background: logSubview === 'sparkies' ? 'linear-gradient(90deg, #975ab6, #ffa3a5)' : '#e5e7eb', color: logSubview === 'sparkies' ? '#fff' : '#374151' }}
                >
                  Sparkies
                </button>
              </div>
                          ...
                          {item.assignType === 'subtask' && (
                            <div className="mb-3">
                              ...
                              <select value={item.parentProject} onChange={(e) => updateBulkItem(idx, 'parentProject', e.target.value)} className="w-full px-3 py-2 border border-gray-300 text-sm" style={{ borderRadius: '9px' }}>
                                ...
                                {tasks.filter(t => !t.completed && String(t.stream_id) === bulkStream).map(t => <option key={t.id} value={t.id}>{t.title}</option>)}
                              </select>
                            </div>
                          )}
              ...
              <div className="bg-gray-50 p-6 shadow-md space-y-4" style={{ borderRadius: '9px' }}>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-400">Contact + rate auto-format into notes on save</span>
                  <button onClick={saveSpark} disabled={!sparkForm.title} className="px-6 py-2 text-sm font-semibold text-white disabled:opacity-50" style={{ borderRadius: '9px', background: 'linear-gradient(90deg, #1a6b62, #1a5e8a)' }}>Felt burny. Might delete later.</button>
                </div>
                <div className="space-y-3 mt-4">
                  <h3 style={{ fontFamily: "'Mynerve', cursive", fontSize: '18px' }} className="text-gray-900">Active sparkies</h3>
                  {sparks.filter(s => s.status === 'active').map(spark => (
                    <div key={spark.id} className="bg-white p-4 shadow-sm" style={{ borderRadius: '9px' }}>
                      <div className="flex justify-between items-start gap-3 mb-2">
                        <h3 style={{ fontFamily: "'Mynerve', cursive", fontSize: '16px' }} className="text-gray-900 flex-1">{spark.title}</h3>
                        <select
                          defaultValue=""
                      ...
                      <p className="text-xs text-gray-400">{spark.start_date && `${spark.start_date} → `}{spark.end_date}{spark.estimated_hours ? ` · ${spark.estimated_hours}h est.` : ''}</p>
                    </div>
                  ))}
                  {sparks.filter(s => s.status === 'active').length === 0 && <p className="text-gray-400 text-sm">No active sparkies yet.</p>}
                </div>
              </div>
              )}
            </div>
              ...
              <div className="bg-gray-50 p-6 shadow-md" style={{ borderRadius: '9px' }}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b">
                  <div>
                    ...
                    <div className="bg-gray-50 p-4" style={{ borderRadius: '9px' }}>
                      <div className="flex items-center gap-3 mb-3 flex-wrap">
                        <p className="text-4xl font-bold text-blue-600 font-mono">{formatTime(timerSeconds)}</p>
                        <select value={timerSubtaskId} onChange={e => setTimerSubtaskId(e.target.value)} className="flex-1 px-2 py-1 border border-gray-300 text-sm bg-white" style={{ borderRadius: '9px', minWidth: '120px' }}>
                          <option value="">Project level</option>
                          {(selectedTask.subtasks || []).map(s => <option key={s.id} value={s.id}>↳ {s.text}</option>)}
                        </select>
                      </div>
                      <div className="flex gap-2">
                        {!timerActive ? (
                          <button onClick={() => { setTimerActive(true); setTimerStartTime(new Date()); }} className="px-6 py-2 bg-green-500 text-white font-bold text-sm flex-1" style={{ borderRadius: '9px' }}>▶ Start</button>
                        ) : (
                          <button onClick={() => {
                            setTimerActive(false);
                            const endTime = new Date();
                            const hours = parseFloat((timerSeconds / 3600).toFixed(2));
                            logTimeEntry(hours, localDateStr(), timerSubtaskId || null, timerStartTime?.toISOString(), endTime.toISOString())
                              .then(() => { setTimerSeconds(0); setTimerStartTime(null); fetchProjects(); })
                              .catch(e => console.error(e));
                          }} className="px-6 py-2 bg-red-500 text-white font-bold text-sm flex-1" style={{ borderRadius: '9px' }}>⏹ Stop</button>
                        )}
                        <button onClick={() => setTimerSeconds(0)} className="px-4 py-2 bg-gray-300 text-gray-700 font-bold text-sm" style={{ borderRadius: '9px' }}>Reset</button>
                      </div>
                    </div>
                    <div className="mt-3 bg-white p-4" style={{ borderRadius: '9px', border: '0.5px solid #e5e7eb' }}>
                      <p className="text-xs font-medium text-gray-600 mb-2">Log time manually</p>
                      <div className="flex gap-2 items-end flex-wrap">
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Date</label>
                          <input type="date" value={manualDate} onChange={e => setManualDate(e.target.value)} className="px-2 py-1 border border-gray-300 text-sm" style={{ borderRadius: '9px' }} />
                        </div>
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Duration (hrs)</label>
                          <input type="number" step="0.25" min="0" value={manualHours} onChange={e => setManualHours(e.target.value)} placeholder="1.5" className="w-20 px-2 py-1 border border-gray-300 text-sm" style={{ borderRadius: '9px' }} />
                        </div>
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Assign to</label>
                          <select value={manualSubtaskId} onChange={e => setManualSubtaskId(e.target.value)} className="px-2 py-1 border border-gray-300 text-sm bg-white" style={{ borderRadius: '9px' }}>
                            <option value="">Project level</option>
                            {(selectedTask.subtasks || []).map(s => <option key={s.id} value={s.id}>↳ {s.text}</option>)}
                          </select>
                        </div>
                        <button
                          onClick={() => {
                            ...
                            logTimeEntry(hours, manualDate, manualSubtaskId || null)
                              .then(() => { setManualHours(''); setManualSubtaskId(''); fetchProjects(); })
                              .catch(e => console.error(e));
                          }}
                          className="px-4 py-1.5 bg-blue-600 text-white font-semibold text-sm"
                          style={{ borderRadius: '9px' }}
                        >Log</button>
                      </div>
                    </div>
                ...
                <div className="mb-6 pb-6 border-b">
                  ...
                  {(() => {
                    const projectEntries = (timeEntries[selectedTask.id] || []).filter(e => !e.subtask_id);
                    return projectEntries.length > 0 ? (
                      <div className="mb-3 bg-gray-50 p-3" style={{ borderRadius: '9px' }}>
                        <p className="text-xs font-medium text-gray-500 mb-2">Project-level time</p>
                        {projectEntries.sort((a, b) => (b.entry_date || '').localeCompare(a.entry_date || '')).map((e, i) => (
                          <div key={i} className="flex items-center gap-3 text-sm py-1">
                            <span className="text-gray-400 w-24 flex-shrink-0">{new Date((e.entry_date || '').split('T')[0] + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                            {e.start_time && e.end_time ? (
                              <span className="text-gray-500 flex-1">{new Date(e.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })} – {new Date(e.end_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</span>
                            ) : <span className="flex-1"></span>}
                            <span className="font-medium text-gray-700">{parseFloat(e.hours || 0).toFixed(2)}h</span>
                          </div>
                        ))}
                      </div>
                    ) : null;
                  ...
                  <div className="space-y-2">
                    {[...(selectedTask.subtasks || [])].sort((a, b) => {
                      ...
                      const subtaskEntries = (timeEntries[selectedTask.id] || []).filter(e => String(e.subtask_id) === String(s.id));
                      const subtaskTotal = subtaskEntries.reduce((sum, e) => sum + parseFloat(e.hours || 0), 0);
                      const isExpanded = expandedSubtaskLogs[s.id];
                      return (
                      <div key={s.id}>
                      <div className="flex items-center gap-3 p-3 bg-white" style={{ borderRadius: '9px' }}>
                        <input
                          type="checkbox"
                          checked={s.completed}
                        ...
                        {editingSubtaskId === s.id ? (
                          <div className="flex-1 flex gap-2 items-center flex-wrap">
                            ...
                            <button onClick={() => saveSubtaskEdit(s.id)} className="px-3 py-1 bg-blue-600 text-white text-sm font-medium" style={{ borderRadius: '9px' }}>Save</button>
                            <button onClick={() => setEditingSubtaskId(null)} className="px-3 py-1 bg-gray-200 text-gray-700 text-sm font-medium" style={{ borderRadius: '9px' }}>Cancel</button>
                          </div>
                        ) : (
                          <div
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                              {s.estimated_hours > 0 && <span className="text-xs text-gray-400">{s.estimated_hours}h est.</span>}
                              {subtaskTotal > 0 && <span className="text-xs text-purple-500">{subtaskTotal.toFixed(2)}h logged</span>}
                              {s.deadline && <span className="text-xs text-gray-400">{daysUntilDeadline(s.deadline)}</span>}
                            </div>
                          </div>
                        )}
                        {subtaskEntries.length > 0 && (
                          <button onClick={() => toggleSubtaskLog(s.id)} className="text-gray-400 hover:text-gray-600 flex-shrink-0" style={{ fontSize: '12px', background: 'none', border: 'none', cursor: 'pointer' }}>
                            {isExpanded ? '▲' : '▼'}
                          </button>
                        )}
                      </div>
                      {isExpanded && subtaskEntries.length > 0 && (
                        <div className="ml-8 mr-2 mb-1 bg-gray-50 px-3 py-2" style={{ borderRadius: '9px' }}>
                          {subtaskEntries.sort((a, b) => (b.entry_date || '').localeCompare(a.entry_date || '')).map((e, i) => (
                            <div key={i} className="flex items-center gap-3 text-sm py-1">
                              <span className="text-gray-400 w-24 flex-shrink-0">{new Date((e.entry_date || '').split('T')[0] + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                              {e.start_time && e.end_time ? (
                                <span className="text-gray-500 flex-1">{new Date(e.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })} – {new Date(e.end_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</span>
                              ) : <span className="flex-1 text-gray-300">manual entry</span>}
                              <span className="font-medium text-gray-700">{parseFloat(e.hours || 0).toFixed(2)}h</span>
                            </div>
                          ))}
                        </div>
          ...
          {currentView === 'smoke' && !selectedTask && (
            <div>
              ...
              <div className="flex items-center gap-3 mb-4 flex-wrap">
                <button
                  onClick={() => { setShowBatchDemote(!showBatchDemote); setBatchDemoteSelected([]); setBatchDemoteParent(''); setShowBatchArchive(false); setBatchArchiveSelected([]); }}
                ...
                >
                  {showBatchDemote ? '✕ Cancel' : 'Batch demote tasks'}
                </button>
                <button
                  onClick={() => { setShowBatchArchive(!showBatchArchive); setBatchArchiveSelected([]); setShowBatchDemote(false); setBatchDemoteSelected([]); }}
                  className="px-4 py-2 text-sm font-medium text-white"
                  style={{ borderRadius: '9px', background: showBatchArchive ? '#888' : 'linear-gradient(90deg, #2a9d8f, #106b73)', boxShadow: showBatchArchive ? 'none' : '0 2px 8px rgba(42,157,143,0.3)' }}
                >
                  {showBatchArchive ? '✕ Cancel' : 'Batch archive completed'}
                </button>
                {showBatchArchive && batchArchiveSelected.length > 0 && (
                  <button
                    onClick={() => { if (window.confirm(`Archive ${batchArchiveSelected.length} task(s) as completed?`)) batchArchive(); }}
                    className="px-4 py-2 text-sm font-semibold text-white"
                    style={{ borderRadius: '9px', background: '#4a918b' }}
                  >
                    Confirm archive ({batchArchiveSelected.length})
                  </button>
                )}
              ...
              {tasks.length === 0 ? (
                ...
                <div className="flex gap-4 overflow-x-auto pb-2">
                  {streams.map(stream => {
                    const colTasks = tasks.filter(t => String(t.stream_id) === stream.id && !t.archived);
                      ...
                      <div key={stream.id} className="flex-shrink-0 w-64 bg-gray-50 p-3" style={{ borderRadius: '9px' }}>
                        ...
                        <div className="space-y-2">
                          {sortedTasks.map(task => {
                            ...
                            const isArchiveSelected = batchArchiveSelected.includes(task.id);
                            return (
                              <div
                                key={task.id}
                                onClick={() => {
                                  if (showBatchDemote && !isCompleted) toggleBatchSelect(task.id);
                                  else if (showBatchArchive && isCompleted) toggleBatchArchiveSelect(task.id);
                                ...
                                style={{
                                  ...
                                  borderRadius: '9px',
                                  background: isSelected ? '#e0f2fe' : isArchiveSelected ? '#dcfce7' : noDeadline ? '#fef08a' : '#fff',
                                  opacity: isCompleted ? 0.7 : 1,
                                  outline: isSelected ? '2px solid #0097b2' : isArchiveSelected ? '2px solid #2a9d8f' : noDeadline ? '2px solid #eab308' : undefined,
                                  borderLeftWidth: '6px',
                              ...
                              >
                                {showBatchDemote && !isCompleted && (
                                  <input type="checkbox" checked={isSelected} onChange={() => toggleBatchSelect(task.id)} onClick={(e) => e.stopPropagation()} className="absolute top-2 right-2 w-4 h-4 accent-blue-600" />
                                )}
                                {showBatchArchive && isCompleted && (
                                  <input type="checkbox" checked={isArchiveSelected} onChange={() => toggleBatchArchiveSelect(task.id)} onClick={(e) => e.stopPropagation()} className="absolute top-2 right-2 w-4 h-4 accent-green-600" />
                                )}
                                ...
                                {!showBatchDemote && !showBatchArchive && (
                                  <button
          ...
          {currentView === 'hose' && !selectedTask && (
            <div key={localDateStr()}>
              ...
              {tasks.length === 0 ? (
                ...
                const definedStartDates = activeTasks
                  .filter(t => t.start_date && t.start_date !== '')
                  .map(t => {
                ...
                let minDate = definedStartDates.length > 0
                  ? new Date(Math.min(...definedStartDates))
                  : new Date(today);
                ...
                activeTasks.forEach(t => {
                  if (t.deadline) {
                    const p = t.deadline.split('T')[0].split('-');
                    deadlineDates.push(new Date(parseInt(p[0]), parseInt(p[1])-1, parseInt(p[2])));
                  }
                  t.subtasks?.forEach(s => {
                    if (s.deadline) {
                      const p = s.deadline.split('T')[0].split('-');
                      deadlineDates.push(new Date(parseInt(p[0]), parseInt(p[1])-1, parseInt(p[2])));
                    }
                  });
                });
                const sixMonthsOut = new Date(today.getFullYear(), today.getMonth() + 6, today.getDate());
                let maxDate = deadlineDates.length > 0
                  ? new Date(Math.max(...deadlineDates))
                  : new Date(sixMonthsOut);
                ...
                const scrollToToday = () => {
                  const scrollPos = Math.max(0, (todayOffset / totalDays) * (totalDays * dayWidth) - 100);
                  if (ganttBodyRef.current) ganttBodyRef.current.scrollLeft = scrollPos;
                  if (ganttHeaderRef.current) ganttHeaderRef.current.scrollLeft = scrollPos;
                };
                ...
                const barStyle = (startDate, endDate, color, progressPct) => {
                  ...
                  const e = endDate ? parseLocalDate(endDate) : new Date(s.getTime() + 86400000);
                      ...
                      if (blackoutDays.includes(date.getDay())) {
                        stripes.push(
                          <div
                            key={`bo-${d}`}
                            className="absolute top-0 bottom-0"
                            style={{ left: `${(d / totalDays) * 100}%`, width: `${(1 / totalDays) * 100}%`, background: `repeating-linear-gradient(${d % 2 === 0 ? '45deg' : '-45deg'}, rgba(0,151,178,0.6), rgba(0,151,178,0.6) 3px, rgba(0,151,178,0.8) 3px, rgba(0,151,178,0.8) 6px)`, zIndex: 0 }}
                          ></div>
                        );
                      }
                    }
                ...
                return (
                  <div className="bg-gray-50 shadow-md" style={{ borderRadius: '9px' }}>
                    {/* Fixed header — scrolls horizontally in sync with body */}
                    <div
                      ref={ganttHeaderRef}
                      style={{ overflowX: 'auto', overflowY: 'hidden', borderBottom: '1px solid #e5e7eb', background: '#f9fafb', borderRadius: '9px 9px 0 0' }}
                      onScroll={e => { if (ganttBodyRef.current) ganttBodyRef.current.scrollLeft = e.target.scrollLeft; }}
                    >
                      <div style={{ width: `${timelineWidth + 240}px`, minWidth: '100%', paddingTop: '4px', paddingBottom: '4px' }}>
                        {/* Month row */}
                        <div style={{ display: 'flex' }}>
                          <div style={{ width: '160px', flexShrink: 0 }}></div>
                          <div className="relative h-7 mb-1" style={{ width: `${timelineWidth}px`, flexShrink: 0 }}>
                            {months.map((m, i) => (
                              <div key={i} className="absolute top-0 bottom-0 flex items-center justify-end text-xs font-medium text-gray-600 border-l border-gray-300 pr-2" style={{ left: `${m.left}%`, width: `${m.width}%` }}>
                                {m.label}
                              </div>
                            ))}
                          </div>
                        ...
                        <div className="flex">
                          <div style={{ width: '160px', flexShrink: 0 }}></div>
                          <div className="relative border-b border-gray-200" style={{ height: '24px', width: `${timelineWidth}px`, flexShrink: 0 }}>
                            <div className="absolute top-0 bottom-0 w-px bg-red-400" style={{ left: `${(todayOffset / totalDays) * timelineWidth}px` }}></div>
                            <span className="absolute text-xs text-white font-medium px-1.5 py-0.5" style={{ left: `${(todayOffset / totalDays) * timelineWidth}px`, transform: 'translateX(-50%)', top: '2px', background: '#E24B4A', borderRadius: '4px', whiteSpace: 'nowrap', zIndex: 3 }}>Today</span>
                          </div>
                          <div style={{ width: '80px', flexShrink: 0 }}></div>
                        </div>
                        {/* Holiday legend */}
                        {holidays.length > 0 && (
                          <div className="flex flex-wrap gap-3 text-xs text-gray-500 pb-1" style={{ paddingLeft: '160px' }}>
                            {holidays.map(h => (
                    ...
                    {/* Scrollable body */}
                    <div
                      ref={el => { ganttBodyRef.current = el; if (el) { const scrollPos = Math.max(0, (todayOffset / totalDays) * (totalDays * dayWidth) - 100); el.scrollLeft = scrollPos; if (ganttHeaderRef.current) ganttHeaderRef.current.scrollLeft = scrollPos; } }}
                      style={{ overflowX: 'auto', overflowY: 'auto', maxHeight: '65vh' }}
                      onScroll={e => { if (ganttHeaderRef.current) ganttHeaderRef.current.scrollLeft = e.target.scrollLeft; }}
                    >
                      <div style={{ width: `${timelineWidth + 240}px`, minWidth: '100%', padding: '4px 16px 16px', position: 'relative' }}>
                        {/* Full-height Today line */}
                        <div style={{ position: 'absolute', top: 0, bottom: 0, left: `${160 + 16 + (todayOffset / totalDays) * timelineWidth}px`, width: '2px', background: '#E24B4A', opacity: 0.4, zIndex: 1, pointerEvents: 'none' }}></div>
                        {/* Sparkies */}
                        {sparks.filter(s => s.status === 'active' && (s.start_date || s.end_date)).map(spark => {
                          const sparkStart = spark.start_date ? spark.start_date.split('T')[0] : minDateStr;
                          const sparkEnd = spark.end_date ? spark.end_date.split('T')[0] : sparkStart;
                          const sparkBar = barStyle(sparkStart, sparkEnd, '#9b8ec4', 0);
                          return (
                            <div key={`spark-${spark.id}`} className="flex items-center gap-2 py-0.5">
                              <div className="w-40 flex-shrink-0 truncate text-xs sticky left-0 pr-2" style={{ zIndex: 2, borderRadius: '4px', paddingLeft: '4px', background: 'hsla(210,17%,98%,0.65)', fontStyle: 'italic', color: '#9b8ec4' }} title={spark.title}>✦ {spark.title}</div>
                              <div className="relative flex-1 flex items-center" style={{ height: '17px' }}>
                                <div className="relative w-full" style={{ height: '8px', background: '#e5e7eb', borderRadius: '4px' }}>
                                  <div onClick={() => setLogSubview('sparkies')} className="absolute top-0 cursor-pointer" style={{ height: '8px', left: `${sparkBar.leftPct}%`, width: `${Math.max(sparkBar.widthPct, 2)}%`, background: 'repeating-linear-gradient(45deg, #9b8ec4, #9b8ec4 4px, #c4bce8 4px, #c4bce8 8px)', borderRadius: '4px', minWidth: '8px', zIndex: 1, opacity: 0.75 }}></div>
                                </div>
                              </div>
                              <div className="w-20 flex-shrink-0 text-xs" style={{ color: '#9b8ec4' }}>{spark.end_date ? daysUntilDeadline(spark.end_date) : ''}</div>
                            </div>
                          );
                        })}
                        ...
                        {activeTasks.map(task => {
                          ...
                          const taskBar = barStyle(getStartDate(task, minDateStr), task.deadline, getStreamColor(task.stream_id), progressPct);
                          ...
                          return (
                            <div key={task.id}>
                              ...
                              {subtasksWithDates.map(sub => {
                                const subStart = sub.start_date ? sub.start_date.split('T')[0] : getStartDate(task, minDateStr);
          ...
          {currentView === 'ashes' && !selectedTask && (
            <div>
              <h2 style={{ fontFamily: "'Ranchers', cursive", fontSize: '24px', color: '#0078a0', marginBottom: '1.5rem' }}>Of Fires Past</h2>
              {(() => {
                ...
                const filteredByDate = filteredByStream.filter(t => {
                  ...
                  const d = t.completed_date.split('T')[0];
                  return d >= dashDateStart && d <= dashDateEnd;
                });
                // Completed subtasks
                const completedSubtasks = tasks.flatMap(t =>
                  (t.subtasks || []).filter(s => s.completed).map(s => ({ ...s, stream_id: t.stream_id, parent_title: t.title }))
                ).filter(s => {
                  if (dashStreamFilter !== 'all' && String(s.stream_id) !== dashStreamFilter) return false;
                  return true;
                });
                ...
                const onTime = filteredByDate.filter(t => t.completed_date && t.deadline && t.completed_date.split('T')[0] <= t.deadline.split('T')[0]).length;
                ...
                return (
                  <div>
                      ...
                      <div className="space-y-2">
                        {filteredByDate.sort((a, b) => (b.completed_date || '').localeCompare(a.completed_date || '')).map(task => {
                          ...
                          return (
                            <div
                              key={task.id}
                              className="flex items-center gap-3 p-3 bg-gray-50 hover:bg-gray-100 cursor-pointer transition-all"
                              style={{ borderRadius: '9px' }}
                              onClick={() => setSelectedTask(task)}
                            >
                              ...
                              <button onClick={(e) => { e.stopPropagation(); uncompleteTask(task.id); }} className="text-xs px-2 py-1 bg-white border border-gray-200 text-gray-500 hover:text-gray-800" style={{ borderRadius: '9px' }}>↩</button>
                            </div>
                          );
                        })}
                    ...
                    {completedSubtasks.length > 0 && (
                      <div>
                        <h3 style={{ fontFamily: "'Mynerve', cursive", fontSize: '18px', marginBottom: '0.75rem' }} className="text-gray-900">Completed subtasks</h3>
                        <div className="space-y-2">
                          {completedSubtasks.map(s => (
                            <div key={s.id} className="flex items-center gap-3 p-3 bg-gray-50" style={{ borderRadius: '9px' }}>
                              <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: getStreamColor(s.stream_id) }}></span>
                              <span className="flex-1 text-sm text-gray-600 line-through">{s.text}</span>
                              <span className="text-xs text-gray-400">↳ {s.parent_title}</span>
                            </div>
                          ))}
                        </div>
                      </div>
      ...
      {showReward && rewardAnimal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" onClick={() => setShowReward(false)}>
          <div className="bg-white p-8 text-center max-w-sm" style={{ borderRadius: '9px' }} onClick={e => e.stopPropagation()}>
            <p style={{ fontFamily: "'Ranchers', cursive", fontSize: '28px' }} className="text-blue-600 mb-1">You did a thing.</p>
            <p style={{ fontFamily: "'Ranchers', cursive", fontSize: '22px' }} className="text-purple-500 mb-4">Good job. Have {rewardAnimal.article} {rewardAnimal.name} or two.</p>
            ...
            <button onClick={() => setShowReward(false)} className="text-sm text-gray-400 hover:text-gray-600">dismiss</button>
          </div>
        </div>
      )}
    </div>
...
export default LifeFiresDashboard;
