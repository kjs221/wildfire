              ...
              <div className="bg-gray-50 p-6 shadow-md" style={{ borderRadius: '9px' }}>
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-6 text-center" style={{ borderRadius: '9px' }}>
                      ...
                      <div className="flex gap-3 justify-center">
                        {!timerActive ? (
                          ...
                          <button onClick={() => { setTimerActive(false); const hours = parseFloat((timerSeconds / 3600).toFixed(2)); fetch(`${SUPABASE_URL}/rest/v1/time_entries`, { method: 'POST', headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' }, body: JSON.stringify({ project_id: selectedTask.id, hours, entry_date: localDateStr(), notes: '' }) }).then(() => { setTimerSeconds(0); fetchProjects(); }).catch(e => console.error(e)); }} className="px-8 py-3 bg-red-500 text-white font-bold text-lg" style={{ borderRadius: '9px' }}>⏹ Stop</button>
                        )}
                ...
                {(timeEntries[selectedTask.id] || []).length > 0 && (
                  <div className="mb-6 pb-6 border-b">
                    <h3 style={{ fontFamily: "'Mynerve', cursive", fontSize: '18px', marginBottom: '0.75rem' }} className="text-gray-900">Time log</h3>
                    <div className="space-y-1">
                      {Object.entries(
                        (timeEntries[selectedTask.id] || []).reduce((acc, e) => {
                          const d = (e.entry_date || '').split('T')[0];
                          acc[d] = (acc[d] || 0) + parseFloat(e.hours || 0);
                          return acc;
                        }, {})
                      ).sort(([a], [b]) => b.localeCompare(a)).map(([date, hours]) => (
                        <div key={date} className="flex items-center gap-3 px-3 py-2 bg-gray-50 text-sm" style={{ borderRadius: '9px' }}>
                          <span className="text-gray-500 w-24 flex-shrink-0">{new Date(date + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                          <div className="flex-1 bg-gray-200 h-2" style={{ borderRadius: '9px' }}>
                            <div className="h-full" style={{ width: `${Math.min(100, (hours / 8) * 100)}%`, background: 'linear-gradient(90deg, #975ab6, #ffa3a5)', borderRadius: '9px' }}></div>
                          </div>
                          <span className="text-gray-700 font-medium w-12 text-right">{hours.toFixed(1)}h</span>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          {currentView === 'hose' && !selectedTask && (
            <div>
              ...
              {tasks.length === 0 ? (
                      ...
                      if (blackoutDays.includes(date.getDay())) {
                        stripes.push(
                          <div
                            key={`bo-${d}`}
                            className="absolute top-0 bottom-0"
                            style={{ left: `${(d / totalDays) * 100}%`, width: `${(1 / totalDays) * 100}%`, background: 'rgba(0,151,178,0.6)', zIndex: 0 }}
                          ></div>
                        );
                      }
                    }
                ...
                return (
                  <div className="bg-gray-50 shadow-md" style={{ borderRadius: '9px', overflow: 'hidden' }}>
                    ...
                    <div style={{ width: `${timelineWidth + 240}px`, minWidth: '100%' }}>
                      ...
                      <div className="space-y-1" style={{ padding: '4px 16px 16px' }}>
