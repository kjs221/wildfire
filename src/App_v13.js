  ...
  const convertToSubtask = async (taskId, parentId) => {
    ...
    try {
      ...
      await fetch(`${SUPABASE_URL}/rest/v1/subtasks`, {
        method: 'POST',
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ project_id: parseInt(parentId), text: selectedTask.title, start_date: selectedTask.start_date || null, deadline: selectedTask.deadline || null, completed: false })
      });
