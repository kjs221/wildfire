...
const WildfireDashboard = () => {
  ...
  const [subtaskDraft, setSubtaskDraft] = useState({ text: '', startDate: '', deadline: '' });
      ...
      await fetch(`${SUPABASE_URL}/rest/v1/subtasks?id=eq.${subtaskId}`,
        {
          method: 'PATCH',
          headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
          body: JSON.stringify({ text: subtaskDraft.text, start_date: subtaskDraft.startDate || null, deadline: subtaskDraft.deadline || null })
        }
      );
  ...
  const navButtons = [
    ...
    { id: 'sanity', label: 'Fire Drill', icon: '🧠' },
  ];
  if (loading) {
      ...
      >
        <div className="bg-white p-4 sm:p-8" style={{ borderRadius: '9px' }}>
          ...
          {currentView === 'fire' && !selectedTask && (
            <div>
              <h2 style={{ fontFamily: "'Indie Flower', cursive", fontSize: '24px', color: '#0078a0', marginBottom: '1.5rem' }}>What's on Fire Today</h2>
                      ...
                      <div key={s.id} className="flex items-center gap-3 p-3 bg-white" style={{ borderRadius: '9px' }}>
                        ...
                        {editingSubtaskId === s.id ? (
                          <div className="flex-1 flex gap-2 items-center flex-wrap">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <input
                                ...
                                onChange={(e) => setSubtaskDraft({ ...subtaskDraft, startDate: e.target.value })}
                          ...
                          <div
                            ...
                            onClick={() => { setEditingSubtaskId(s.id); setSubtaskDraft({ text: s.text, startDate: s.start_date ? s.start_date.split('T')[0] : '', deadline: s.deadline ? s.deadline.split('T')[0] : '' }); }}
                            title="Click to edit"
                          >
          ...
          {currentView === 'sanity' && !selectedTask && (
            <div>
              <h2 style={{ fontFamily: "'Indie Flower', cursive", fontSize: '24px', color: '#0078a0', marginBottom: '1.5rem' }}>Fire Drill — Manage Streams</h2>
              {(() => {
                ...
                const totalUsed = tasksInRange.reduce((sum, t) => sum + (t.estimated_hours || 0), 0);
                ...
                return (
                  <div className="bg-gray-50 p-6 shadow-md mb-4" style={{ borderRadius: '9px' }}>
                    <div className="flex items-center gap-3 mb-3 flex-wrap">
                      <h3 style={{ fontFamily: "'Indie Flower', cursive", fontSize: '18px' }} className="text-gray-900">Cumulative capacity</h3>
                    ...
                    <div className="w-full bg-gray-200 h-4 overflow-hidden" style={{ borderRadius: '9px' }}>
                      <div className="h-full" style={{ width: `${totalPct}%`, background: totalOver ? '#E24B4A' : '#1D9E75', borderRadius: '9px' }}></div>
                    </div>
                    ...
                    <div style={{ width: `${timelineWidth + 240}px`, minWidth: '100%' }}>
                      ...
                      <div className="space-y-1">
                        {tasks.map(task => {
                          ...
                          const subtasksWithDates = (task.subtasks || []).filter(s => s.deadline || s.start_date);
                          return (
                            <div key={task.id}>
                              <div className="flex items-center gap-2 py-0.5">
                                ...
                                <div className="relative flex-1" style={{ height: '17px', background: '#e5e7eb', borderRadius: '4px' }}>
                                  {renderHolidayStripes()}
                                  <div
                                    onClick={() => setSelectedTask(task)}
                                    className="absolute top-0 cursor-pointer flex items-center px-2"
                                    style={{ height: '17px', left: `${taskBar.leftPct}%`, width: `${Math.max(taskBar.widthPct, 2)}%`, background: taskBar.color, borderRadius: '4px', minWidth: '8px', zIndex: 1 }}
                                  >
                              ...
                              {subtasksWithDates.map(sub => {
                                ...
                                return (
                                  <div key={sub.id} className="flex items-center gap-2 py-0.5">
                                    ...
                                    <div className="relative flex-1" style={{ height: '11px', background: '#f3f4f6', borderRadius: '4px' }}>
                                      {renderHolidayStripes()}
                                      <div
                                        onClick={() => setSelectedTask(task)}
                                        className="absolute top-0 cursor-pointer"
                                        style={{ height: '11px', left: `${subBar.leftPct}%`, width: `${Math.max(subBar.widthPct, 1.5)}%`, background: subBar.color, borderRadius: '4px', minWidth: '6px', zIndex: 1, opacity: sub.completed ? 0.5 : 1 }}
                                      ></div>
                                    </div>
